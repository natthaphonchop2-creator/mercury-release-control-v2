...
def _surface_evidence() -> dict[str, object]:
    surfaces = []
    for name in V030_SURFACE_NAMES:
        surfaces.append(
            {
                "blocker_codes": [],
...
def test_attestation_is_exact_fresh_and_sanitized() -> None:
    ...
    assert attestation.surface_count == len(V030_SURFACE_NAMES)
...
def test_attestation_rejects_surface_count_mismatch() -> None:
    attestation = assemble_attestation(
        evidence=_provider_evidence(),
        preflight=_preflight(),
        staging=_staging(),
    ...
    altered = attestation.model_copy(update={"surface_count": 8})
    altered = altered.model_copy(
        update={
            "payload_sha256": _payload_digest(
                altered.model_dump(exclude={"payload_sha256"})
            )
        }
    )
